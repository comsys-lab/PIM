...
class Drambuffer:
    ...
    def os_dataflow(self, params):
        """Return dram access count and stall cycles"""
        input = 0
        filter = 0
        output = 0#len(input_operand) * len(filter_operand[0])

        stall_cycle = 0


        input_total = [["[-1,-1,-1]"] for i in range(params.systolic.row)]
        filter_total = [["[-1,-1,-1]" for i in range(params.systolic.col)]]

        if params.input_operand.row % params.systolic.row != 0:
            input_operand = self.BaseOperation.input_padding(params.systolic,params.input_operand)
        if params.filter_operand.col % params.systolic.col != 0:
            filter_operand = self.BaseOperation.filter_padding(params.systolic,filter_operand)
